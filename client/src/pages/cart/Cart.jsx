  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          type="button"
          className="w-fit rounded-full border border-gray-300 bg-white px-5 py-2 text-xs font-medium text-[#111] transition-colors hover:border-[#111]"
      ...
      {!items.length ? (
        <div className="flex min-h-[45vh] flex-col items-center justify-center rounded-[28px] bg-[#f6f6f6] px-6 text-center">
          ...
          <p className="mt-3 max-w-lg text-base text-gray-600">Add a product from Men&apos;s Shoes and choose a size to see it here.</p>
          <button
            type="button"
          ...
          <div className="space-y-4">
            {items.map((item) => (
              <div key={`${item._id}-${item.size}`} className="flex flex-col gap-4 rounded-[28px] bg-[#f6f6f6] p-4 sm:flex-row">
                <div className="h-40 w-full overflow-hidden rounded-[24px] bg-white sm:w-40 sm:flex-none">
